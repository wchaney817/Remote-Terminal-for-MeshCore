...
- `GET /packets/recent?since=<unix_ts>&limit=<n>&payload_types=<csv>` — **personal-fork addition, not upstream.** Backfills the live packet-feed UI from persisted history on page load; `RawPacketBroadcast` rows oldest-first, synthetic negative `observation_id`. Registered before `/{packet_id}` so it isn't swallowed by that route. Optional `payload_types` (backend `PayloadType` enum names, e.g. `GROUP_TEXT,TEXT_MESSAGE`) filters server-side by scanning up to `PAYLOAD_TYPE_FILTER_SCAN_LIMIT` rows (payload type isn't a stored column) — `limit` then counts matches, not rows scanned. The frontend's payload-type checkboxes use a *different* decoder's naming (`@michaelhart/meshcore-decoder`, e.g. `GroupText`) than this backend enum, so `RawPacketFeedView.tsx` maps checkbox labels to enum names before calling this; `GROUP_DATA`/`ANON_REQUEST`/`MULTIPART`/`RAW_CUSTOM` exist on the backend but have no frontend checkbox and so can never be requested via that UI.
